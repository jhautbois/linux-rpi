...
static int ub960_write_ind8(struct ub960_data *priv, u8 block, u8 reg, u8 val)
{
	int ret;
...
static int ub960_write_ind16(struct ub960_data *priv, u8 block, u8 reg, u16 val)
{
	...
	ret = ub960_write_ind8(priv, block, reg, val >> 8);
	if (ret)
		return ret;

	return ub960_write_ind8(priv, block, reg + 1, val & 0xff);
}
				 ...
				 u8 mask, u8 val)
{
	...
	ret = ub960_read_ind(priv, block, reg, &v);
	if (ret)
		return ret;
	...
	return ub960_write_ind8(priv, block, reg, v);
}
					...
					unsigned int nport, s8 strobe_pos)
{
	...
	ub960_write_ind8(priv, UB960_IND_TARGET_RX_ANA(nport),
					       ...
					       struct v4l2_subdev_state *state)
{
	...
	dev_dbg(dev, "ub960_configure_ports_for_streaming\n");
...
static void ub960_enable_tpg(struct ub960_data *priv, int tpg_num)
{
	...
	ub960_write_ind8(priv, UB960_IND_TARGET_PAT_GEN, UB960_IR_PGEN_CTL,
			 UB960_IR_PGEN_CTL_PGEN_ENABLE);

	/* YUV422 8bit: 2 bytes/block, CSI-2 data type 0x1e */
	ub960_write_ind8(priv, UB960_IND_TARGET_PAT_GEN, UB960_IR_PGEN_CFG,
			 cbars_idx << 4 | 0x2);
	ub960_write_ind8(priv, UB960_IND_TARGET_PAT_GEN, UB960_IR_PGEN_CSI_DI,
			 0x1e);
	...
	ub960_write_ind8(priv, UB960_IND_TARGET_PAT_GEN, UB960_IR_PGEN_VBP,
			 vbp);
	ub960_write_ind8(priv, UB960_IND_TARGET_PAT_GEN, UB960_IR_PGEN_VFP,
			 vfp);
}
...
static void ub960_disable_tpg(struct ub960_data *priv)
{
	...
	ub960_write_ind8(priv, UB960_IND_TARGET_PAT_GEN, UB960_IR_PGEN_CTL, 0x00);
}
...
static int ub960_probe(struct i2c_client *client)
{
	...
	priv->current_read_rxport = priv->current_write_rxport_mask = 0xff;
	priv->current_read_csiport = priv->current_write_csiport_mask = 0xff;
